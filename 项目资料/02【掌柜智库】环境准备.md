...
创建新的虚拟机：典型 > iso > 选择虚拟机名称和位置（kb） > 磁盘大小（40G） > 自定义硬件（内存:4G，CPU:2x2）> Install CentOS7

安装设置：中文 | 简体中文 | 日期时间（修改正确的东八区时间） | 安装位置（直接确认） | 网络和主机（手动配置IPv4到VMNet8虚拟网段，不要使用网段、广播地址和宿主机地址）

开始安装：同时 设置ROOT密码

网络和主机名可以安装CentOS的时候配置，也可以服务器安装完毕后以命令行方式修改：
...
#    显式指定 --console-address ":9001" 可避免端口随机化
#
# 3. 数据安全：
...
# 访问密钥
#vfRRNCknp2VimgOmGlaw
MINIO_ACCESS_KEY=minioadmin 
# 私有密钥
#AzXccngMWBPKKHK9NgQu2vDEq74WapkCycWJbAAc
...
<img src="images/02-uv1.png" alt="02-uv1" style="zoom:70%;" />

#### 方式2

`PyCharm`版本没有 `uv` 选项，则选择 `自定义环境`，类型选 `uv`，然后再点击 `安装 uv via pip`

<img src="images/02-uv2.png" alt="02-uv2" style="zoom:70%;" />
...
<img src="images/02-PowerShell.png" alt="02-PowerShell" style="zoom:70%;" />

如果运行报如下错误：

<img src="images/02-error.jpg" alt="02-error" style="zoom: 80%;" />
...
<img src="images/02-创建项目.png" alt="02-创建项目" style="zoom:70%;" />
...
![image-20260629060306172](images/02-激活虚拟环境.png)
...
> `.env.example` 文件（提交到Git，不要配置真实的 apikey，避免泄露）
